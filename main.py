...
def get_data_info():
	...
	# test_count = len(datagen.test_files_list)
...
def dataset_Filename():
	...
	# if test_count:
	# 	test_generator = datagen.build_test_generator
	# 	test_dataset = tf.data.Dataset.from_generator(train_generator, \
	# 				output_types=(tf.string), output_shapes=(None))
	# 	return_dataset = return_dataset+tuple(test_dataset)
...
def dataset_Feature_Raw():
	...
	# test_dataset = tf.data.TFRecordDataset(os.path.join(config["data"]["tfrecord"]["directory"], config["data"]["tfrecord"]["test_filename"]))
...
def parse_map(example_proto):
	...
	with tf.name_scope("parsing_train_val_samples"):
		...
		one_hot_label = tf.reshape(one_hot_label, tf.cast([tf.cast(height, tf.int64), tf.cast(width, tf.int64), 2], dtype=tf.int64))
...
def main():
	...
	if config["phase"] == "prepare_data":
		...
		# tfRecGen = TFRecordGenerator(config["data"]["directory"]+"/test", None)
		# data_file = os.path.join(config["data"]["tfrecord"]["directory"], config["data"]["tfrecord"]["test_filename"])

		# if (not os.path.isfile(data_file)) or (os.stat(data_file).st_size==0):
		# 	tfRecGen.write_TFRecord_test(config["data"]["tfrecord"]["test_filename"])
		# else:
		# 	utils.print_log("testing tfrecord already exists", "INFO")
			# tfRecGen.read_TFRecord_test(config["data"]["tfrecord"]["test_filename"])
		...
		with tf.name_scope("input") as scope:
			...
			# print(current_data.input.shape)
			# print(current_data.label.shape)
			# print(is_training)
		...
		if config["training_setting"]["analyse_lr"]:
				...
				try:
					for train_index in range(int(train_count/batch_size)):
						...
						if (cnt == (total_steps-1)*batch_size) or ((cnt-batch_size) % int(config["training_setting"]["logging"]["display_iteration"]) == 0):
							...
							# _, current_loss, cur_data = sess.run([train_op, loss, current_data], feed_dict={handle: train_handle, is_training:True})
							...
							# out = sess.run(logits, feed_dict={handle: train_handle, is_training:True})
							# print(out.shape)
						...
						if cnt>0 and cnt % int(config["training_setting"]["logging"]["evaluation_iteration"]) == 0:
							...
							if best_miou < m_iou:
								...
								if config["training_setting"]["save_visualization_image"]:
									for g in range(output_logits.shape[0]):
										out_logits_sample = output_logits[g,:,:,:]
										out_image = utils.reverse_one_hot(out_logits_sample)
										out_vis_image = utils.color_code_segmentation(out_image, setting.label_values)
										scipy.misc.imsave(config["training_setting"]["checkpoints"]["save_directory"]+"/"+config["model"]+"/"+ckpt_dir+"/segmentation_images/step_"+str(g)+".png", out_vis_image)
			...
			#run the training along with validation and save the checkpoints
			#do class balancing if required
			#summary writing to be implemented

	elif config["phase"] == "test":
		# _, __, test_dataset = dataset_Feature_Raw()
		saver = tf.train.import_meta_graph("./model_graphs/adapnet.meta")
		saver.restore(sess, tf.train.latest_checkpoint(config["training_setting"]["checkpoints"]["save_directory"]+"/"+config["model"]+"/run_2018_07_30_10:25:33"))
		batch_size = int(config["training_setting"]["batch_size"])

		handle = tf.get_default_graph().get_tensor_by_name("input/handle:0")
		logits = tf.get_default_graph().get_tensor_by_name("output/AdapNet/model_prediction:0")
		phase = tf.get_default_graph().get_tensor_by_name("input/phase:0")
		#initialise the test iterator
		test_dataset = test_dataset.map(parse_map_test)
		test_dataset = test_dataset.batch(batch_size)
		test_iterator = test_dataset.make_one_shot_iterator()
		test_handle = sess.run(test_iterator.string_handle())
		output_logits = sess.run(logits, feed_dict={handle:test_handle, phase:"test"})

		print(output_logits.shape)
		for i in range(sess.run(tf.shape(output_logits)[0])):
			out_image = output_logits[i,:,:,:]
			out_image = utils.reverse_one_hot(out_image)
			out_vis_image = utils.color_code_segmentation(out_image, setting.label_values)
			cv2.imwrite("%s/%s_pred.png"%("test_results", str(i)),cv2.cvtColor(np.uint8(out_vis_image), cv2.COLOR_RGB2BGR))


if __name__=="__main__":
	setting.init()

	if check_argument_sanity():
		get_data_info()
		main()
